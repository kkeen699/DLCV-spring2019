...
for epoch in range(1, N_EPOCH+1):
    ...
    data_tar_iter = iter(target_trainloader)
    ...
    while i < len_dataloader + 1:
        ...
        class_output, domain_output = model(x=s_img, alpha=alpha)
        ...
        _, domain_output = model(x=t_img, alpha=alpha)
    ...
    if (epoch) % 5 == 0:
        torch.save(model.state_dict(), './s_u/model_epoch_%d.pth' % (epoch))
